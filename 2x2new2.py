...
def edges_on_from_plan(plan_steps):
    """
    CHANGED:
    - Original looked for action name == "Draw"
    - Now action names are Draw_0, Draw_1, ... so we detect name.startswith("Draw_")
    """
    on = set()
...
@lru_cache(None)
def degree_0_or_2_cached(edges_tuple):
    return degree_0_or_2(list(edges_tuple))
...
domain = set(
    map(
        ...
        + [step_name(i) for i in range(E + 1)],
    )
)
...
background = set(
    map(
        ...
        (
            ...
            # CHANGED: declare steps (optional typing facts)
            + [f"(Step {step_name(i)})" for i in range(E + 1)]
        ),
    )
)
...
start.add(r(f"(Ready {step_name(0)})"))
...
# vertex goals (cached string building)
vertex_goals = [degree_0_or_2_cached(tuple(vtx_incident[v])) for v in vertices]

# non-empty loop so at least one edge must be drawn.
nonempty_goal = "(or " + " ".join(f"(On {e})" for e in edges) + ")"

# CHANGED: must finish processing all edges (reach final step)
processed_goal = f"(Ready {step_name(E)})"

# final goal
all_goals = [clue_goal_str] + vertex_goals + [nonempty_goal, processed_goal]
