...
def build_grid(H, W):
    ...
    for r0 in range(H):
        for c0 in range(W):
            incident[cell_name(r0, c0)] = [
                h_name(r0, c0),  # top
                h_name(r0 + 1, c0),  # bottom
                v_name(r0, c0),  # left
                v_name(r0, c0 + 1),  # right
            ]
...
def build_vertices(H, W):
    ...
    for r0 in range(H + 1):
        for c0 in range(W + 1):
            ...
            if c0 > 0:
                inc.append(h_name(r0, c0 - 1))
            if c0 < W:
                inc.append(h_name(r0, c0))

            # verticals at this vertex
            if r0 > 0:
                inc.append(v_name(r0 - 1, c0))
            if r0 < H:
                inc.append(v_name(r0, c0))
...
def print_slitherlink_ascii(H, W, on_edges, clues):
    ...
    for r0 in range(H):
        ...
        line = dot
        for c0 in range(W):
            line += (HBAR if h_name(r0, c0) in on_edges else SPACE) + dot
        print(line)

        # cell row r
        line = ""
        for c0 in range(W):
            line += VBAR if v_name(r0, c0) in on_edges else " "
            cell = cell_name(r0, c0)
            line += f" {clue_char[clues[cell]]} " if cell in clues else SPACE
        line += VBAR if v_name(r0, W) in on_edges else " "
        print(line)

    # bottom boundary
    line = dot
    for c0 in range(W):
        line += (HBAR if h_name(H, c0) in on_edges else SPACE) + dot
    print(line)
...
# -------------------------
# CHANGED: helper for connectivity goals
# -------------------------
def all_off_at_vertex(edges_at_vertex) -> str:  # CHANGED
    if len(edges_at_vertex) == 0:
        return "(and)"
...
# -------------------------
# CHANGED: domain includes vertices too (for Reach predicates)
# -------------------------
domain = set(map(r, edges + vertices))  # CHANGED

# -------------------------
# CHANGED: you MUST give facts for Incidence, otherwise Propagate can't fire
# (keeping background minimal but sufficient)
# -------------------------
...
background |= set(
    map(
        r,
        [f"(Inc {v} {e})" for v in vertices for e in vtx_incident[v]],
    )
)  # CHANGED
...
actions = [
    Action(
        r("(Draw ?e)"),
        precondition=r("(and (Edge ?e) (not (On ?e)))"),  # CHANGED (safer than just (not (On ?e)))
        additions={r("(On ?e)")},
...
# -------------------------
# CHANGED: reachability propagation action
# -------------------------
...
# -------------------------
# CHANGED: initialize Reach
# pick a fixed root vertex (top-left)
# -------------------------
root = "p00"  # CHANGED
...
# -------------------------
# CHANGED: connectivity goals to force ONE loop
# If a vertex has any On edge, it must be Reach-able from root.
# Together with degree 0/2, this kills multiple disjoint cycles.
# -------------------------
conn_goals = []  # CHANGED
for v in vertices:  # CHANGED
...
# final goal
all_goals = [clue_goal_str] + vertex_goals + conn_goals + [nonempty_goal]  # CHANGED
